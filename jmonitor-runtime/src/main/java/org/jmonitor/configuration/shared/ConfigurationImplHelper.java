 ...
 * @since 1.0
 */
// cannot put these methods directly inside impl classes (as constructors or statics)
// because the impl classes are used by GWT client and so cannot depend on spring BeanUtils
public final class ConfigurationImplHelper {

    // utility class
    private ConfigurationImplHelper() {
    }
    ...
    public static MetricConfigurationImpl copyOf(MetricConfiguration configuration) {
        MetricConfigurationImpl copy = new MetricConfigurationImpl();
        for (String metricName : configuration.getMetricNames()) {
            for (String propertyName : configuration.getPropertyNames(metricName)) {
                copy.setProperty(metricName, propertyName, configuration.getProperty(metricName,
                        propertyName));
            }
        }
        return copy;
    ...
    public static FullConfigurationImpl copyOf(FullConfiguration configuration) {
        ...
        copy.setMetricConfiguration(copyOf(configuration.getMetricConfiguration()));
