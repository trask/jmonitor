 ...
 * as public methods. Original authors Rod Johnson and Juergen Hoeller.
 * 
...
public final class ExpirationUtils {
    ...
    private static final boolean USE_EXPIRES_HEADER = true;

    /** Use HTTP 1.1 cache-control header? */
    private static final boolean USE_CACHE_CONTROL_HEADER = true;

    /** Use HTTP 1.1 cache-control header value "no-store"? */
    private static final boolean USE_CACHE_CONTROL_NO_STORE = true;
    ...
    public static void preventCaching(HttpServletResponse response) {
        ...
        if (USE_EXPIRES_HEADER) {
            // HTTP 1.0 header
            response.setDateHeader(HEADER_EXPIRES, 1L);
        }
        if (USE_CACHE_CONTROL_HEADER) {
            // HTTP 1.1 header: "no-cache" is the standard value,
            // "no-store" is necessary to prevent caching on FireFox.
            response.setHeader(HEADER_CACHE_CONTROL, "no-cache");
            if (USE_CACHE_CONTROL_NO_STORE) {
