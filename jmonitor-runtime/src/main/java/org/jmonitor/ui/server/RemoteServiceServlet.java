...
/**
 * The servlet base class for your RPC service implementations that automatically deserializes
 * incoming requests from the client and serializes outgoing responses for client/server RPCs.
 * 
 * Original authors the google-web-toolkit team.
 * 
 * @author Trask Stalnaker
 * @since 1.0
 */
public class RemoteServiceServlet extends AbstractRemoteServiceServlet implements
    ...
    static SerializationPolicy loadSerializationPolicy(HttpServlet servlet,
        ...
        if (modulePath == null || !modulePath.startsWith(contextPath)) {
            ...
            // MODIFIED THIS LINE TO NOT PREPEND contextRelativePath
            // Trask 4/28/10
            String serializationPolicyFilePath = SerializationPolicyLoader
                    .getSerializationPolicyFileName("org.jmonitor.ui.Monitor/" + strongName);

            // Open the RPC resource file and read its contents.
            // MODIFIED THIS LINE TO USE ClassLoader INSTEAD OF ServletContext
            // Trask 4/28/10
            InputStream is = RemoteServiceServlet.class.getClassLoader().getResourceAsStream(
    ...
    public final SerializationPolicy getSerializationPolicy(String moduleBaseURL,
        ...
        serializationPolicy = doGetSerializationPolicy(getThreadLocalRequest(), moduleBaseURL,
                strongName);
     ...
     * {@link RPC#invokeAndEncodeResponse(Object, java.lang.reflect.Method, Object[])} method to do
     * the actual work.
     * <p>
     * Subclasses may optionally override this method to handle the payload in any way they desire
     * (by routing the request to a framework component, for instance). The
     * {@link HttpServletRequest} and {@link HttpServletResponse} can be accessed via the
     * {@link #getThreadLocalRequest()} and {@link #getThreadLocalResponse()} methods.
     * </p>
     * This is public so that it can be unit tested easily without HTTP.
     * 
     * @param payload
     *            the UTF-8 request payload
     * @return a string which encodes either the method's return, a checked exception thrown by the
     *         method, or an {@link IncompatibleRemoteServiceException}
     * @throws SerializationException
     *             if we cannot serialize the response
     * @throws UnexpectedException
     *             if the invocation throws a checked exception that is not declared in the service
     *             method's signature
     * @throws RuntimeException
     *             if the service method throws an unchecked exception (the exception will be the
     *             one thrown by the service)
     */
     ...
     * 
     * This doPost method swallows ALL exceptions, logs them in the ServletContext, and returns a
    ...
    /**
     * Gets the {@link SerializationPolicy} for given module base URL and strong name if there is
     * one.
     * 
     ...
     * @return a {@link SerializationPolicy} for the given module base URL and strong name, or
     ...
     * The default implementation does nothing and need not be called by subclasses.
     * 
    ...
    /**
     * Override this method to examine the serialized version of the request payload before it is
     * deserialized into objects. The default implementation does nothing and need not be called by
     * subclasses.
     * 
    ...
    /**
     * Determines whether the response to a given servlet request should or should not be GZIP
     * compressed. This method is only called in cases where the requester accepts GZIP encoding.
     * <p>
     * This implementation currently returns <code>true</code> if the response string's estimated
     * byte length is longer than 256 bytes. Subclasses can override this logic.
     * </p>
     * 
     * @param request
     ...
     * @return <code>true</code> if responsePayload should be GZIP compressed, otherwise
     *         <code>false</code>.
     */
