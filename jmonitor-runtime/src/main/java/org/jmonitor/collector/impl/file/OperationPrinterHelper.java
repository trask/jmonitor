...
class OperationPrinterHelper {
    ...
    private static final Ordering<SampledCallTreeNode> sampledCallTreeNodeOrdering =
            new Ordering<SampledCallTreeNode>() {
                public int compare(SampledCallTreeNode node1, SampledCallTreeNode node2) {
                    // reverse sort
                    return node2.getSampleCount() - node1.getSampleCount();
                }
            };

    private static final Ordering<MetricDataItem> metricDataItemOrdering =
            new Ordering<MetricDataItem>() {
                public int compare(MetricDataItem o1, MetricDataItem o2) {
                    return o1.getName().compareToIgnoreCase(o2.getName());
                }
            };

    private static final Ordering<Entry<State, Integer>> threadStateOrdering =
            new Ordering<Entry<State, Integer>>() {
                public int compare(Entry<State, Integer> o1, Entry<State, Integer> o2) {
    ...
    public void logOperation(int maxTraceElements) {
        ...
        if (completedAsOfLogNanoTime) {
            long durationInMilliseconds = TimeUnit.NANOSECONDS.toMillis(operation
                    .getDurationInNanoseconds());
            Date endTimeDate = new Date(operation.getStartTime().getTime()
        ...
        writeExecutionTraceRootElement();

        // write metric data items
        writeMetricDataItems(operation.getMetricData().getItems());

        // write contextual trace elements
        writeExecutionTrace(maxTraceElements);

        // write sampled call tree
        writeSampledCallTree();
    }
    ...
    private void writeExecutionTraceRootElement() {

        FormatUtils.printHeader(out, "operation summary");

        writeContextMap(operation.getExecutionTrace().getRootElement().getContext(), 0);
        out.println();
    }

    private void writeContextMap(ProbeExecutionContext contextMap, int level) {

        for (Entry<String, String> entry : contextMap.getMap().entrySet()) {
        ...
        for (Entry<String, ProbeExecutionContext> entry : contextMap.getNestedMaps().entrySet()) {
    ...
    private void writeExecutionTrace(int maxTraceElements) {
        ...
        String rootDuration = formatDurationInMilliseconds(operation.getExecutionTrace()
                .getRootElement());
        int durationColumnWidth = rootDuration.length();

        int count = 0;
        int lastIndex = -1;
        for (ExecutionTraceElement traceElement : operation.getExecutionTrace().getElements()) {

            if (maxTraceElements != AgentConfiguration.TRACE_ELEMENT_LIMIT_DISABLED
                    && count++ >= maxTraceElements) {
                break;
            }

            if (traceElement.getIndex() != lastIndex + 1) {
                // the +1 accounts for the "+" at the beginning of the offset times
                FormatUtils.printPadding(out, startTimeOffsetColumnWidth
                        - DOT_DOT_DOT.length() + 1);
                out.println(DOT_DOT_DOT);
            }
            lastIndex = traceElement.getIndex();

            // print offset time (in seconds)
            String offsetTimeText = formatOffsetTimeInSeconds(traceElement);
            ...
            out.print("  ");
            String durationText = formatDurationInMilliseconds(traceElement);
            FormatUtils.printPadding(out, 2 * traceElement.getLevel() + durationColumnWidth
                    - durationText.length());
            out.print("(");
            ...
            // print description
            out.print("  ");
            out.println(traceElement.getDescription());
        }
    ...
    private String formatOffsetTimeInSeconds(ExecutionTraceElement traceElement) {
        long startNanoTimeOffset = traceElement.getOffsetInNanoseconds();
        return FormatUtils.formatNanosecondsAsSeconds(startNanoTimeOffset);
    }

    private String formatDurationInMilliseconds(ExecutionTraceElement traceElement) {
        if (traceElement.isCompleted()) {
            return FormatUtils.formatNanosecondsAsMilliseconds(traceElement
                    .getDurationInNanoseconds());
        } else {
            return FormatUtils.formatNanosecondsAsMilliseconds(logNanoTime
                    - operation.getStartNanoTime() - traceElement.getOffsetInNanoseconds()) + "..";
        }
    }

    public void writeSampledCallTree() {

        Iterable<? extends SampledCallTreeNode> rootNodes = operation.getSampledCallTree()
                .getRootNodes();

        // TODO this conditional may not be needed in the future if SampledCallTree itself
        // is lazy instantiated
        if (rootNodes == null) {
            return;
        }

        List<? extends SampledCallTreeNode> sortedRootNodes =
                sampledCallTreeNodeOrdering.sortedCopy(rootNodes);

        totalSampleCount = 0;
        for (SampledCallTreeNode rootNode : sortedRootNodes) {
        ...
        double largestPercentage = sortedRootNodes.get(0).getSampleCount()
    ...
    private void writeSampledTreeCallNode(SampledCallTreeNode node, String indent,
            String recurseIndent) {

        printSampledCallTreePercentageColumn(node.getSampleCount());
        out.print(indent);
        ...
        List<? extends SampledCallTreeNode> sortedChildNodes = sampledCallTreeNodeOrdering
                .sortedCopy(node.getChildNodes());

        if (sortedChildNodes.size() == 0) {
            return;
        }
        ...
        for (Iterator<? extends SampledCallTreeNode> i = sortedChildNodes.iterator(); i.hasNext();) {
            SampledCallTreeNode childNode = i.next();
            if (i.hasNext()) {
    ...
    private void printLeafNode(SampledCallTreeNode leafNode, String indent) {

        List<Entry<State, Integer>> sortedEntries = threadStateOrdering
                .sortedCopy(leafNode.getLeafThreadStateSampleCounts().entrySet());
        ...
        for (Entry<State, Integer> entry : sortedEntries) {
            // print percentage
            printSampledCallTreePercentageColumn(entry.getValue());
            out.print(indent);
    ...
    private void printSampledCallTreePercentageColumn(int leafCount) {
    ...
    public void writeMetricDataItems(Iterable<? extends MetricDataItem> items) {
        ...
        // sort by name
        List<? extends MetricDataItem> sortedItems = metricDataItemOrdering.sortedCopy(items);
        ...
        String[] headers = new String[] { TOTAL_HEADER, AVERAGE_HEADER, MINIMUM_HEADER,
                MAXIMUM_HEADER, COUNT_HEADER };
        ...
        for (MetricDataItem item : sortedItems) {
            ...
            rows[i][j++] = millisecondsFormat.format(item.getTotalTimeInNanoseconds()
                    / NANOSECONDS_PER_MILLISECOND);
            rows[i][j++] = millisecondsFormat.format(item.getAverageTimeInNanoseconds()
                    / NANOSECONDS_PER_MILLISECOND);
            rows[i][j++] = millisecondsFormat.format(item.getMinimumTimeInNanoseconds()
                    / NANOSECONDS_PER_MILLISECOND);
            rows[i][j++] = millisecondsFormat.format(item.getMaximumTimeInNanoseconds()
                    / NANOSECONDS_PER_MILLISECOND);
