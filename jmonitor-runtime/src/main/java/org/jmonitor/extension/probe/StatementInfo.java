...
public class StatementInfo {
    ...
    // this doesn't apply to PreparedStatementInfo
    private Collection<String> batchedSql;

    // we store lastExecutedJdbcTraceElement so that we can navigate to the
    // JdbcTraceElementSafeImpl
    // inside the advice for ResultSet.next()
    // in order to increment the JdbcTraceElementSafeImpl field.
    //
    // PreparedStatementInfo objects are cached as long as the application
    // server caches the PreparedStatement
    // so we use a weak reference here to allow the JdbcTraceElementSafeImpl to be collected
    ...
    private WeakReference<ProbeExecution> lastProbeExecution;
    ...
    public ProbeExecution getLastProbeExecution() {
    ...
    public void setLastProbeExecution(ProbeExecution probeExecution) {
        this.lastProbeExecution = new WeakReference<ProbeExecution>(probeExecution);
    }
}
