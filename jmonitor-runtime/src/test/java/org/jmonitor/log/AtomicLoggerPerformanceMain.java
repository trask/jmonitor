...
public final class AtomicLoggerPerformanceMain {
    ...
    private void speedTest(int totalMegabytes, int bytesPerPrintCall) throws IOException {
        ...
        double log4jSpeed = runSpeedTestLog4j(numberOfPrintCalls, line);
        double rawSpeed = runSpeedTestRaw(numberOfPrintCalls, line);

        System.out.println(totalMegabytes + " megabytes at " + bytesPerPrintCall
                + " bytes per print call");
        System.out.println("  log4j speed: " + log4jSpeed + " seconds");
        System.out.println("  raw speed:   " + rawSpeed + " seconds");
        NumberFormat format = NumberFormat.getPercentInstance();
        format.setMaximumFractionDigits(1);
        format.setMinimumFractionDigits(1);
        String percentageText = format.format((log4jSpeed - rawSpeed) / log4jSpeed);
        System.out.println("  raw is " + percentageText + " faster");
    }

    private double runSpeedTestRaw(int numberOfPrintCalls, String printText) throws IOException {
        ...
        runSpeedTest(out, numberOfPrintCalls, printText);
        double rawSpeed = (System.currentTimeMillis() - startMillis)
    ...
    private double runSpeedTestLog4j(final int numberOfPrintCalls, final String printText)
            throws IOException {

        // TODO delete this file at the end of the test
        File file = File.createTempFile(TEMP_FILE_PREFIX, TEMP_FILE_SUFFIX);
        ...
        double log4jSpeed =
                (System.currentTimeMillis() - startMillis) / (double) MILLISECONDS_PER_SECOND;
